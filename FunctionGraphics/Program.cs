...
namespace FunctionGraphics
{
    class Program
    {
        ...
        public static double minBound, maxBound;
        private static void ConsoleHandler()
        {
            while (true)
            {
                ...
                if (cmd == "draw func")
                {
                    ...
                    double step = Parser<double>.Read("Step", double.TryParse);
                    minBound = Parser<double>.Read("Min bound", double.TryParse);
                    maxBound = Parser<double>.Read("Max bound", double.TryParse);
